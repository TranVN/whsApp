...
<?php 
    ...
    if(isset($_GET['tentho'])){
        ...
        try {
            ...
            work_do.sum_chi, work_do.sum_thu,work_do.note_work, info_cus.flag_status 
            FROM work_do , info_cus, info_worker 
            WHERE  info_cus.flag_book = 1 
            and work_do.id_cus = info_cus.id_cus 
            and work_do.id_worker = info_worker.id_worker 
            and work_do.sum_thu = 0 and info_worker.name_worker like '%$tentho%' ORDER BY info_cus.date_book DESC";
    ...
    else 
    {
        try {
            ...
            and work_do.id_cus = info_cus.id_cus 
            and work_do.id_worker = info_worker.id_worker 
            and work_do.sum_thu = 0  ORDER BY info_cus.date_book DESC";
            $result= $conn->query($sql);
            $result ->setFetchMode(PDO::FETCH_ASSOC);
            $result->execute();  
            $num = $result ->rowCount();
    ...
    <table class='table table-bordered table-hover'>
         ...
         <tbody>";
          for( $i = 0 ; $i < count( $results->data ); $i++ ) : 
                      ...
                      <td>".$results->data[$i]['date_book']."</td> 
                      ...
                      <td>".$results->data[$i]['sum_thu']."</td>
                            ...
                            echo"<a href ='".BASE_URL."includes/logic/deleteKH.php?hd=ks&id_cus=".$results->data[$i]['id_cus']."'class='btn btn-sm btn-primary' "; if($results->data[$i]['sum_thu'] > 0 || $results->data[$i]['flag_status']=='Khảo Sát'){echo "disabled";}  echo " >Khảo sát</a>";
                            echo "&nbsp";
                            echo"<button type='button' class='btn btn-sm btn-warning' data-toggle='modal' data-target='#my".$results->data[$i]['id_cus']."' "; if($results->data[$i]['sum_thu'] > 0 || $results->data[$i]['flag_status']=='Hủy'){echo "disabled";}  echo ">Hủy Lịch</button>
